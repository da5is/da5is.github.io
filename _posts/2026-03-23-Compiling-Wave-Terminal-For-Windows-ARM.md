...
2.  Install the required build dependencies if you don't have them already.
    1.  `winget install task.task` - winget detects and installs the ARM version of task; Scoop does not as of 2026-03-23.
    2. `scoop install go` - scoop remains my prefered method of installing software on Windows 
    3. `scoop install zig`
...
4.  At this point, the 'BUILD.md' instructions from the repository should complete successfully - so, following those, you just need to run `task init` and `task package`. 